...
object StructClassValidatorExtension : FirClassChecker(MppCheckerKind.Common) {

    @OptIn(SymbolInternals::class)
    context(context: CheckerContext, reporter: DiagnosticReporter)
    override fun check(declaration: FirClass) {
        if (!declaration.annotations.hasAnnotation(AnnotationClassIds.STRUCT, context.session)) {
            return
        }
